...
**Status (2026-07-14): WP-4 implemented, tested, and independently audited — approved and committed.** Implemented exactly as scoped above: `generate_weekly_summary()` computes the requested ISO week's 7 calendar days, excludes any day at or after today (UTC) entirely from the roll-up ("Not yet closed" — §11, never a still-open day's partial data), and for each closed day either parses its already-written Daily Summary file (reading only the leading integer of each `- <Label>: <N>...` bullet, ignoring any parenthetical disposition detail) or — if no file exists — consults the action log (via WP-1's `filter_entries_by_day()`) solely to distinguish "No activity" (zero entries, a real known zero) from "Report unavailable for this date" (entries exist but generation never ran — excluded from totals, never silently zeroed, per §12 Layer 1's L1 fix). Never calls `storage.database.load_metadata_store()` at all — Weekly Summary's only sources are Daily Summary files and the action log (§5). No independent week-boundary or closed-period mechanism introduced: a fully-closed week's file is naturally byte-identical on every re-run since its only inputs (individually G6-protected Daily Summary files) can't change once written — inherited, not re-implemented, per the WP-4 pre-implementation audit's own conclusion. No other file touched beyond `reporting.py` and its test file, confirmed via `git diff` against the WP-3 commit — `runtime_io.py`'s `write_weekly_summary()` remains exactly as WP-1 left it. Tests: `pipeline/test_reporting.py`, 17 new WP-4 tests (multi-day roll-up against real `generate_daily_summary()` output, parenthetical-detail parsing robustness, both disambiguation sub-cases, ISO year-boundary, not-yet-closed exclusion, determinism, malformed-line disclosure, zero-write including a structural check that the metadata store is never even touched). Full regression suite: 677/677, confirmed no real `Database/`/`Runtime/` file touched. Independent implementation audit: zero Critical/High/Medium findings; two Low (the missing-day existence check's private-attribute reach into `runtime_io._RUNTIME_REPORTS_PATH`, the same disclosed pattern already accepted for WP-2, now recurring; the Markdown-bullet-parsing coupling to WP-2's exact rendering format — an intentional, design-level coupling per §9's own "avoid two parallel aggregation paths" reasoning, not a defect, but worth naming for future maintainers since nothing enforces it at the type level). Seven accepted Low findings total across WP-1 through WP-4.
...
**Storage Report scope resolved (2026-07-20) — metadata-store-only, current aggregate confirmed.** Full record: `Governance/ARCHITECTURE_DECISIONS.md` decisions 28 (growth-over-time clarified as a current aggregate, no persisted history) and 29 (OD-4 resolved: metadata-store-only); `Module 08 Design.md`'s "Addendum — Storage Report Scope & Data Source Resolution." Per this project's "never rewrite history" convention, the conditional prose above is left unedited; this note confirms which branches are now operative. The Storage Report aggregates existing `size_bytes` from `Database/Metadata/metadata_store.json` by destination folder/category, computed fresh on every call, with no persisted history of any kind — the "if OD-4 → real filesystem measurement" branch, its adversarial-filesystem-test requirement, and its "new small helper plus `KNOWN_LIMITATIONS.md` disclosure" clause are no longer applicable. `write_storage_report()` needs no signature change (`content: str -> str`, no scoping parameter, already correct since WP-1). WP-5 now scopes to exactly `reporting.py` and `runtime_io.py`, on its confirmed **Low-Medium** risk branch. WP-5 is unblocked and implementation-ready pending its own pre-implementation audit.

**Inclusion predicate confirmed (2026-07-20) — decision 30.** WP-5's own pre-implementation engineering review surfaced one further scope question not addressed by decisions 25/28/29: which records contribute to storage totals. Resolved: only `record.processed_at is not None` (grouped by `suggested_destination`/`category`) — `review_required`, unreadable, not-yet-approved, and undone records are excluded. Full record: `Governance/ARCHITECTURE_DECISIONS.md` decision 30. WP-5's pre-implementation review is now complete — every open scope question is resolved; implementation may begin pending explicit project-owner approval.
...
**Scope finalized (2026-07-20) — decision 26's auto-hook branch is moot; CLI-placement question resolved by decision 31.** Decision 26 (recorded at WP-0, 2026-07-14) already eliminated this package's "Medium risk, touches `execute()`" branch entirely — `report()` is unconditionally the explicit-CLI-step shape, so WP-6 never touches `execution.py` or any other Module 07 file, under any circumstance; "Files expected to change" above is unconditionally just `main.py`/`test_main.py`. A further question WP-6's own pre-implementation review surfaced — whether `report()` joins `main.py`'s automatic `if __name__ == "__main__":` chain — is now resolved: it does not. Full record: `Governance/ARCHITECTURE_DECISIONS.md` decision 31; `Module 08 Design.md`'s "Addendum — WP-6 `report()` CLI Invocation Placement." `report()` invokes all four `generate_*()` functions in one pass (decision 26), each independently isolated per §12 Layer 2, using UTC "today"/the current ISO week for `generate_daily_summary(date)`/`generate_weekly_summary(week)`'s own scoping parameters (a disclosed, WP-6-level rendering choice, not a new decision — mirrors the same `datetime.now(timezone.utc).date()` pattern those functions already use internally). WP-6 is architecturally frozen and implementation-ready pending explicit approval to begin coding.
