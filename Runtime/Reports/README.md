...
- `Storage Report/` — current space used per destination folder/category: a cumulative aggregate over every currently-filed file (`processed_at is not None` — never a review-required, unreadable, not-yet-approved, or undone record), computed fresh from the metadata store on every run (not a persisted history or trend line — see `Governance/ARCHITECTURE_DECISIONS.md` decisions 28 and 30). Useful for noticing e.g. Screenshots quietly eating disk space.
