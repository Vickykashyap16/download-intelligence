...
## 28. Storage Report's "growth over time" responsibility: satisfied by a current aggregate, never persisted historical snapshots

**Context:** `Module 08 Design.md` §3 describes Storage Report as showing "space used per destination folder/category, growth over time" — phrasing inherited from the pre-design note (`08 Logging & Reporting.md`) and echoed in `Runtime/Reports/README.md`. Decision 25 has since fixed Storage Report's persistence shape as a single, continuously-updated current-state file, explicitly rejecting dated snapshots. A literal "growth over time" reading implies comparing state across points in time, which requires persisted history — a real tension with decision 25.

**Decision:** "Growth over time" is satisfied by Storage Report's own current aggregate totals per category/destination, computed fresh from the metadata store on every invocation. No historical snapshot, rolling/bounded snapshot window, or run-to-run delta is persisted anywhere, and no new `Database/`/`Runtime/` structure or hidden state is introduced.

**Why it was chosen:** Decision 25 already rejected dated snapshots for this exact report type on "avoid over-engineering" grounds (decisions 3/12); §15 confirms Module 08 introduces no new `Database/` structure — a claim this decision preserves rather than reopens. G5's unconditional idempotency guarantee is cleanest as a pure function of the current, unchanged metadata store, not of execution history — the same reasoning §7's own rejected Option (C), a monotonic run counter, already applied ("a new piece of state Module 08 would need to track and persist somewhere — in direct tension with §15"). I5 names only the action log and metadata store as legitimate trace sources, not a report's own prior output or a new state file. Under this project's own "never delete" non-negotiable, a current aggregate over every file ever discovered already is a cumulative, historical rollup — matching NG4's own text that Storage Report "show[s] historical rollups only; no projection of future growth or behavior."

**Trade-offs:** No point-in-time trend or growth visualization is possible from this report alone — a reader cannot see whether usage grew or shrank between two runs, only its current state. Accepted, mirroring decision 25's identical trade-off; a future, genuinely-motivated need for real growth tracking is its own new, explicit decision, not preempted here.

**Consequences:** `Module 08 Design.md` §3's original text is left unedited (append-only convention); a dated addendum clarifies its meaning without altering it. `Module 08 Implementation Plan.md` WP-5's aggregation scope is unchanged by this decision — it resolves an ambiguity in what WP-5 already intended, not new logic. Establishes the settled report shape that OD-4 (decision 29) resolves the data source for.

---

## 29. Storage Report's data source (OD-4 resolved): derived entirely from `size_bytes` already in the metadata store, never a real filesystem walk

**Context:** `Module 08 Design.md` §3/§5/§18/§22 (Open Decision OD-4) left open whether Storage Report's per-folder/per-category figures are derived entirely from `size_bytes` values already present on every `FileRecord` in `Database/Metadata/metadata_store.json`, or computed by walking the real destination library's folder structure on disk — a new, disclosed read-only filesystem dependency (§18). Unlike OD-1/OD-3/OD-5, OD-4 was deliberately left deferred through WP-0 and every Design Review/Implementation Plan Review round, and remained the one substantive gap blocking WP-5. Decision 28 has since fixed Storage Report's scope as a current aggregate over the metadata store's own already-existing fields, removing any growth-tracking rationale that might otherwise have argued for live, repeated filesystem measurement.

**Decision:** The Storage Report is derived entirely from `size_bytes` values already present in `Database/Metadata/metadata_store.json`, aggregated by destination folder and category. Module 08 never reads the real destination library's folder structure, never performs a filesystem walk, and never opens a filed file to measure it. `write_storage_report()` requires no new dependency and no scoping parameter beyond what decision 25 already settled.

**Why it was chosen:** `size_bytes` is set once by Module 01 at discovery and never modified by any later module (Module 07 only changes `current_path`) — no code path in this pipeline currently causes a filed file's real size to diverge from its recorded value. G5's idempotency guarantee and I5's trace-source wording both name only the action log/metadata store, not live filesystem state, as legitimate sources; a filesystem-measurement answer would require extending I5, an undisclosed consequence nothing in the design anticipates. G7 already frames the filesystem-read exception as conditional ("for the Storage Report only, see OD-4") rather than committed, and this decision declines to exercise it. This keeps Module 08's dependency footprint at exactly what WP-2/WP-3/WP-4 already established (`Database/`, `Runtime/Logs/`) — the smallest option consistent with decisions 3/12's "avoid over-engineering, defer genuinely useful but non-essential capability" discipline, and reuses decision 20's `destination_root` precedent not at all, since no filesystem path resolution is needed under this branch. It lands WP-5 on its own certified "Low-Medium" risk branch (`Module 08 Implementation Plan.md`) rather than its "High," first-of-its-kind filesystem-dependency branch. Decision 28 removes the one plausible reason to prefer live measurement (tracking change over time) by settling that Storage Report is a current aggregate, not a trend — real filesystem measurement would only ever protect against a scenario (post-filing size drift) this pipeline has no mechanism to cause.

**Trade-offs:** If a filed file is ever modified in place after execution — outside this pipeline's own visibility, since v1 has no mechanism that causes or observes this — the Storage Report reflects its size as of discovery, not current on-disk reality. Accepted, since no code path currently causes this and no motivated need for protecting against it has been named. A future, genuinely-motivated need for live measurement is its own new, explicit decision, not preempted here.

**Consequences:** `Module 08 Implementation Plan.md` WP-5's conditional branching resolves onto its metadata-store-only, Low-Medium-risk path — the real-filesystem-measurement branch, its adversarial-filesystem-test requirement, and its "new small helper plus `KNOWN_LIMITATIONS.md` disclosure" clause no longer apply. WP-5 is unblocked and implementation-ready pending its own pre-implementation audit.

---

## 30. Storage Report's inclusion predicate: only currently-filed records (`processed_at is not None`) contribute to storage totals

**Context:** Decision 29 established that Storage Report derives entirely from `size_bytes` values already present in the metadata store, but neither `Module 08 Design.md` §3/§5/§19 nor decisions 25/28/29 specify *which* records count toward "space used per destination folder/category." `suggested_destination` (Module 05) is populated for every classified record regardless of whether Module 07 ever actually executed a move — including `review_required` records, which `Rules/Folder Rules.md` guarantees are "not moved at all," and any record still awaiting an approval decision. Grouping every classified record by `suggested_destination` regardless of execution status would report space as already consumed in a destination folder a file was never actually placed in — surfaced during WP-5's own pre-implementation engineering review, not previously named anywhere in the frozen design.

**Decision:** A record contributes to Storage Report's totals if and only if `record.processed_at is not None`, grouped by `suggested_destination` (destination-folder breakdown) and `category` (category breakdown), using `size_bytes` exactly as decision 29 already scoped. `review_required` records, unreadable records, records still awaiting approval, and records whose disposition was later undone (`processed_at` reset to `None` by `undo_single_action()`) are excluded.

**Why it was chosen:** Confirmed directly in `src/pipeline/execution.py`: `record.processed_at` is written in exactly one place, `ExecutionEngine.execute_file()` step 6, immediately after a real, successful `move_rename`/`archive_duplicate`/`archive_superseded_version`, and is reset to `None` by `undo_single_action()` — an already-existing, metadata-store-only signal requiring no new field, no action-log read, and no filesystem access, fully consistent with decision 29's "entirely from the metadata store" framing. This is the only predicate that keeps every reported figure traceable to a file's real, current, physical location (G3/I5) rather than to a merely-suggested one: `Module 08 Design.md` §0.1 I2 guarantees `review_required` records are "left completely unchanged," so treating one as if it already occupies its suggested category folder would be a fabricated figure, not a real one. It also makes the report self-correcting under undo with no special-case logic — an undone disposition's record already reverts to `processed_at is None` and falls out of the totals on the very next call, matching decision 25's "always-current view" philosophy exactly the way `generate_duplicate_report()` already re-classifies an undone archive back to "Kept."

**Trade-offs:** A file that has been classified, named, and approved but not yet executed contributes nothing to Storage Report, even though it will very shortly occupy real space — accepted, since counting it before it exists at that location would be exactly the "growth" projection NG4 already disclaims, not a current aggregate. A record archived under `~ARCHIVE~/Duplicates/`/`~ARCHIVE~/Old Versions/` still counts under its own destination-folder bucket, since it is real, currently-occupied space in the user's library — this is a deliberate inclusion, not an oversight.

**Consequences:** `Module 08 Design.md`'s "Addendum — Storage Report Scope & Data Source Resolution" gains one further, dated resolution bullet (append-only). `Module 08 Implementation Plan.md` WP-5's own status note is updated to state the confirmed inclusion predicate explicitly. `Runtime/Reports/README.md`'s Storage Report line, which previously read "a cumulative aggregate over every file discovered so far," is corrected to accurately describe currently-filed space only — a factual correction, not a re-litigation, since that wording was written before this predicate was confirmed. WP-5's pre-implementation blueprint's one open ambiguity is now closed; nothing else in that blueprint changes.

---

## 31. WP-6 `report()` CLI invocation placement: an explicitly-invoked command, never part of the automatic `python -m src.main` execution chain

**Context:** Decision 26 resolved OD-3 (report generation is its own explicit CLI step, `report()` in `src/main.py`, never an auto-hook into `execute()`), but neither that decision nor `Module 08 Design.md` §10/§22 nor `Module 08 Implementation Plan.md`'s WP-6 entry addressed a distinct, narrower question: whether `report()` should be appended to `main.py`'s existing `if __name__ == "__main__":` chain (`scan() → classify() → extract() → detect_duplicates() → suggest_naming() → score_confidence()`), which deliberately stops short of `execute()`/`undo()` today because those cause real filesystem moves or require a `batch_id`. `report()` causes neither, so its placement in or out of that chain was genuinely open — surfaced during WP-6's pre-implementation engineering review, not previously named anywhere in the frozen design or implementation plan.

**Decision:** `report()` is a separate, explicitly-invoked CLI command, exactly like `execute()`/`undo()` — never appended to the automatic chain at the bottom of `main.py`.

**Why it was chosen:** Reports are intended to summarize completed execution results, not the state of a batch still mid-pipeline. Daily Summary's own content (auto-filed/approval-required/review-required/duplicates/versions/errors, the "## Files" table) is derived from `move_rename`/`archive_duplicate`/`archive_superseded_version`/`score_confidence` action-log entries — none of which exist yet at the point `score_confidence()` finishes in the chain, since `execute()` hasn't run. Running `report()` before `execute()` would generate technically correct but systematically premature Daily Summary output on every default `python -m src.main` invocation — an honest zero (G3 is not violated), but a misleading default impression of "today's" activity, undermining the report's practical purpose. Keeping `report()` explicit preserves a predictable workflow (report only after a human has decided execution for the day is done) and avoids this misleading default, mirroring `execute()`/`undo()`'s own existing "consequential enough to require deliberate invocation" precedent — extended here to a new kind of consequence (informational prematurity) rather than a filesystem write.

**Trade-offs:** A user must remember to call `report()` separately after `execute()`, exactly as they already must for `execute()`/`undo()` themselves — no new operational burden, and consistent with `Module 08 Design.md` NG1 ("invoked... Manual or Scheduled... never a live dashboard or push notification").

**Consequences:** `main.py`'s `if __name__ == "__main__":` block remains exactly `scan() → classify() → extract() → detect_duplicates() → suggest_naming() → score_confidence()`, unchanged. WP-6 implements `report()` as a standalone function only, never adds a call site to that block. This decision governs CLI orchestration only — it changes nothing about report-generation logic itself; decisions 25/26/27/28/29/30 and `generate_daily_summary()`/`generate_weekly_summary()`/`generate_duplicate_report()`/`generate_storage_report()` are entirely unaffected. WP-6's pre-implementation blueprint's one open question is now closed; WP-6 is architecturally frozen and implementation-ready.

---

## 32. C1 (Real CLI Entry Point) classified as severity **High** under the existing scale — no new "Product Enhancement" tier introduced

**Context:** C1's own design package (`Build-out/09 CLI & Product Interface/C1 CLI Entry Point — Design Package.md` §1.5) is not a post-freeze defect correction — nothing was broken, no pattern of incorrect behavior was observed — so `ENGINEERING_CHANGE_PLAYBOOK.md`'s Observation/Pattern/Root-Cause vocabulary doesn't fit it cleanly. The design package's first draft resolved this by inventing a new label, "Product Enhancement — High," rather than either picking the closest fit on the existing scale or formally extending it. The Engineering Review (`C1 CLI Entry Point — Engineering Review.md` finding F3) flagged this as a real, if small, consistency risk: `ENGINEERING_STANDARD.md` §14 / `FROZEN_MODULE_CHANGE_POLICY.md` §2 already define one severity scale (Critical/High/Medium/Low/Cosmetic), used consistently across every prior tracking document (`PATTERN_TRACKER.md`, `TECHNICAL_DEBT_REGISTER.md`, `VERSIONS.md`, every `RELEASE_NOTES.md`) — an ad hoc sibling label invented inside one design document, left unresolved, would eventually surface as a "severity doesn't agree everywhere" gap at whatever point this change is tracked or closed alongside PT-002/PT-003-style entries.

**Decision:** C1 is classified as **High** severity, using the existing scale directly — no new severity tier or label is introduced. The design-package depth this implies (full package, one independent review round, per `ENGINEERING_CHANGE_PLAYBOOK.md` §3's Critical/High row) is exactly what C1's design package and Engineering Review already did in practice; only the label itself is corrected by this entry, not the process already followed.

**Why it was chosen:** High is the closest existing-scale fit, not a compromise: C1 introduces new, user-facing capability and becomes the system's primary operational entry point (every module is reached through it going forward), which is more architectural surface than a Low/Medium item, but it carries zero pipeline-logic risk (pure composition over already-frozen, already-tested Module 01–08 functions, plus one disclosed, zero-behavior-change rename — decision detail below) and touches no module's contract, which is less risk than a genuine Critical. A distinct "feature work" severity track was considered (matching the design package's own original instinct) and rejected for now: this project has exactly one data point of this kind so far, not enough evidence to justify a permanent new category over reusing the existing one; if a second, sufficiently different feature-scale change arrives later and the existing scale strains to describe it, that is the point to reconsider — not preemptively here.

**Trade-offs:** "High" was originally defined with defect-severity connotations (how bad is it that this is broken); reusing it for a change where nothing is broken is an imperfect fit, accepted as smaller than the cost of a second, parallel, seldom-used scale that every future tracking document would then need to reconcile against the first.

**Consequences:** `Build-out/09 CLI & Product Interface/C1 CLI Entry Point — Design Package.md` §1.5 is superseded by this entry for severity-labeling purposes (the design package's own text is left as-is per the append-only convention; this decision is the authoritative record going forward). Any future tracking-document entry for C1 (`CHANGELOG.md`, `Release/VERSIONS.md`, a future `PROJECT_BACKLOG.md` update) should cite "High" against the existing scale, not "Product Enhancement." Separately, and for the same "record the reasoning, don't just apply it" reason: the one disclosed touch to already-shipped code C1 required — renaming `main.py`'s `_eligible_for_execution_records()` to the public `eligible_for_execution_records()` so `src/cli.py` could reuse it instead of duplicating its four-condition filter (design package §1.3 item 2, Engineering Review finding F2) — was a deliberate choice between two real options, not a forced one: Python permits importing a leading-underscore name explicitly across a module boundary (`from src.main import _eligible_for_execution_records`), which would have required touching `main.py` not at all. The rename was chosen anyway, because a permanent dependency on a name `main.py` marks as internal is a worse long-run trade than a one-line, zero-behavior-change, fully regression-tested rename — consistent with this project's preference (PT-002/PT-003) for eliminating duplicated-logic drift risk over minimizing lines touched.

## 33. H5 (Configuration Experience) classified as severity **High**, same tier as C1 and for the same reason; diagnostic/exit-code classification applied consistently across its new code paths

**Context:** H5 (`Build-out/09 CLI & Product Interface/H5 Configuration Experience — Design Package.md`) is, like C1, net-new capability rather than a defect correction — `init` and `config --set-source`/`--set-destination` add a guided way to set two settings `src/config/sources.yaml` has always exposed (decision 20), and `scan`/`run` gain a narrow, specific catch that reclassifies an unmet configuration precondition as an expected outcome rather than the generic Layer 3 "Unexpected error" framing (C1-PAT-1). The design package's own Engineering Review (`H5 Configuration Experience — Engineering Review.md` finding F2) found the design package never stated a severity classification at all — the same category of gap decision 32 above already corrected once for C1's design, now recurring by omission rather than by an invented label.

**Decision:** H5 is classified as **High** severity, using the existing scale directly, same tier as C1 (decision 32) and for the same underlying reason. In addition — resolving F2's broader instruction to "explicitly define and consistently apply the project's severity classification to all new diagnostics and exit paths" — every new code path H5 introduces is classified into exactly one of three kinds, applied consistently across `src/cli.py`'s new functions:

1. **An unmet configuration precondition is an *expected*, not an *error*, state.** `_run_scan_with_friendly_config_errors()` catching `ValueError`/`NotADirectoryError` around `run_scan()` (not configured; configured path doesn't exist) always exits 0 with a plain, specific message — no "Error:" framing — matching every other wrapped `main.py` function's own "nothing to do yet" philosophy (C1 design package §3.5's exit-code table). `_cmd_init`/`_cmd_config`'s own "could not find `sources.yaml`" checks follow the same rule, matching `_cmd_version`'s/the original `_cmd_config`'s already-established precedent.
2. **Invalid interactive input is not an error at all — it re-prompts.** `_prompt_path()`/`_confirm()` never raise or exit on bad input; they loop, exactly matching `_prompt_decision()`'s own no-implicit-default discipline (C1 Engineering Review finding F1, `execute`'s interactive loop) extended here to path/confirmation input.
3. **A genuine config-write failure is a real, unanticipated problem, not a user precondition.** `_write_config_value()` raising (the expected line not found; the file not matching the expected shape; write-then-verify mismatch after every safeguard) is deliberately *not* caught anywhere in `_cmd_init`/`_cmd_config` — it propagates to `main()`'s existing Layer 3 net (exit 1, sanitized message unless `--debug`), exactly like any other unanticipated internal error, since by this point normal validation (FR-2/`_resolve_existing_directory()`) has already ruled out ordinary user mistakes.

**Why it was chosen:** High matches C1's own precedent exactly — new, user-facing capability with real product leverage (the Release Planning Matrix's own "single highest-leverage item in this whole review" framing for the underlying finding), zero pipeline-logic risk (composition over already-tested functions, plus one small, self-contained, thoroughly-tested config writer). The three-way diagnostic classification above is not a new severity scale — it is the same expected/precondition-vs-error distinction `ARCHITECTURE_DECISIONS.md` decisions 18/19 (error-handling and fallback philosophy) already establish for the pipeline's own Layer 1/2/3 discipline, applied explicitly and by name to this work package's new surface rather than left to be inferred per function.

**Trade-offs:** None beyond decision 32's own already-accepted trade-off (reusing a defect-connotated scale for non-defect work). Explicitly not introduced here: a distinct exit code for "ran to completion but nothing was actually fixed" (e.g. a scripted caller wanting to know a `scan` didn't proceed) — this is the same open question C1's own §3.5 already disclosed and left unresolved for `execute -y`; H5 inherits it rather than resolving it, and the inheritance itself is now recorded explicitly (Engineering Review finding F6) rather than presented as newly settled.

**Consequences:** Any future tracking-document entry for H5 should cite "High" against the existing scale. The three-way diagnostic classification above is the authoritative reference for how any *future* new `src/cli.py` code path should be classified (expected precondition / re-promptable input / genuine failure) — a future contributor adding a new command should classify its own failure modes against this same three-way split rather than inventing a fourth category.
