...
- **Module08 (Logging & Reporting) generates reports only — it does not write to the action log.** `Runtime/Logs/action_log.jsonl` is written by every module from 01 through 07 as it runs (`src/storage/runtime_io.py`'s `append_action_log()`), starting with Module 01's own `discover`/`skip`/`error` actions; Module 08's own `report()` (`Governance/ARCHITECTURE_DECISIONS.md` decisions 25/26/31) only reads that log and the metadata store back, writing exclusively to `Runtime/Reports/*`. This diagram shows the primary `FileRecord` enrichment order — which module is responsible for filling in which fields — not literal call/execution order.
