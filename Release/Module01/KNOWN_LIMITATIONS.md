...
- **`content_hash` is overwritten to `None` during an "unreadable" status episode** (both before and after post-freeze correction #1, 2026-07-11 — pre-existing, not introduced by that fix). If a file becomes unreadable on one scan (hash set to `None`) and then readable again on a later scan with genuinely different content in between, `build_file_record()`'s `content_changed` detection compares against a `None` previous hash and will not fire — the changed-content reset branch is skipped, and the record's downstream fields are instead preserved as if content hadn't changed. Surfaced during the targeted Independent Implementation Audit of post-freeze correction #1; classified Low severity (requires a fairly specific unreadable-then-readable-with-different-content sequence) and out of scope for that correction. Not yet fixed — worth a future look if unreadable/permission-flapping files turn out to be common in practice.
